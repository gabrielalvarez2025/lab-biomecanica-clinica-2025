...
def mostrar():
    ...
    **Juega con ellos** para ajustar la amplitud, frecuencia y fase de cada onda y **mira cómo van cambiando su forma**:
    ...
    st.markdown(f"""
                Las ondas individuales representan las unidades motoras (UM) que se suman para formar una señal EMG compuesta. Cuando las {num_ondas} ondas ocurren simultáneamente (chocan, se combinan), interfieren entre sí constructivamente en algunas zonas y destructivamente en otras.
                El resultado de sumar gráficamente esas {num_ondas} ondas que configuraste se muestra en la siguiente **onda resultante**:
    """)
